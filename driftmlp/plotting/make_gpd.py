...
def network_to_multipolygon_df(network: igraph.Graph, discretizer=DefaultSystem):
    ...
    if "rotation" in network.attributes():
        h3_df["rotated_centroid"] = h3_df.apply(
            lambda x: Point(
                network["rotation"](x.centroid_col.x, x.centroid_col.y, inverse=True)
            ),
            axis=1,
        )
    else:
...
def full_multipolygon_df(discretizer=DefaultSystem):
    coords_lat, coords_lon = np.meshgrid(
        np.linspace(-90, 90, 3000), np.linspace(-180, 180, 3000)
    )
    h3_list = [
        discretizer.geo_to_ind(lon=lon, lat=lat)
