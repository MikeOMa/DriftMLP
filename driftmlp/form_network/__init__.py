...
def nested_lists_of_count_to_pandas_edgelist(nested_lists_of_counters, h3_ids):
    ...
    data = [
        (counter[col] / n_obs, n_obs, row, col)
        for row, (counter, n_obs) in enumerate(zip(nested_lists_of_counters, sum_iter))
        for col in counter
    ]
    ...
    pd_edgelist = pd.DataFrame(
        {"source": source, "target": target, "prob": probs, "N": n_obs}
    )
    return pd_edgelist
...
def count_transitions(list_of_stories: List, unique_ids: List, **kwargs):
    ...
    id_to_int[-1] = -1
    list_of_stories_int = [
        [id_to_int[key] for key in story] for story in list_of_stories
    ]
...
def pandas_to_igraph(pd_edge_df, pd_vertex_df):
    unique_ids = list(
        set(pd_edge_df["source"].to_list() + pd_edge_df["target"].to_list())
    )
...
def make_transition(
    list_of_h3_stories: List[List[int]], day_cut_off: int = 5, observations_per_day=4
) -> igraph.Graph:
    """
    ...
    list_of_counters = count_transitions(
        list_of_h3_stories,
        unique_ids,
    ...
    pd_edgelist = nested_lists_of_count_to_pandas_edgelist(list_of_counters, unique_ids)

    ##We need negative log probabilities, easiest to do it here.
    pd_edgelist["neglogprob"] = pd_edgelist.apply(lambda x: -np.log(x["prob"]), axis=1)
    ## N current contains vertex counts, replace it with edge observation counts.
    pd_edgelist["N"] = pd_edgelist.apply(lambda x: np.round(x["N"] * x["prob"]), axis=1)
